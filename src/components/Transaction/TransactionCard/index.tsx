	...
	return (
		<li id={id} className='shadow-[0px_2px_8px_1px_rgba(224,232,240,0.4)] bg-neutral-light rounded-md px-3 py-2 flex items-center justify-between max-w-[24.25rem] gap-3'>
			<div className='flex gap-3 max-w-[calc(100%-8.5rem)]'>
				<figure
					className={`h-8 min-w-[2rem] rounded-full grid place-items-center ${type === TransactionType.DEBIT ? 'bg-secondary-light' : 'bg-primary-light'}`}
				>
					{
			...
			<small className={`text-xs font-medium whitespace-nowrap ${type === TransactionType.DEBIT ? 'text-secondary-dark' : 'text-primary'}`}>{transactionValue}</small>
		</li>
	)
}
